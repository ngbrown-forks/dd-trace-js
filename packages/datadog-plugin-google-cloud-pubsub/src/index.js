...
const messageSpans = require('../../datadog-instrumentations/src/google-cloud-pubsub')
class GoogleCloudPubsubPlugin extends Plugin {
  ...
  constructor (...args) {
    ...
    this.addSub(`apm:google-cloud-pubsub:end`, () => {
      this.exit()
    })

    this.addSub(`apm:google-cloud-pubsub:error`, err => {
      const span = storage.getStore().span
      span.setTag('error', err)
    })

    this.addSub(`apm:google-cloud-pubsub:async-end`, () => {
      const span = storage.getStore().span
      span.finish()
    })

    this.addSub(`apm:google-cloud-pubsub:request:add`, ({ cfg }) => {
      const span = storage.getStore().span
    ...
    this.addSub(`apm:google-cloud-pubsub:receive:end`, () => {
      this.exit()
    })
    ...
    this.addSub(`apm:google-cloud-pubsub:receive:async-end`, ({ message }) => {
      const span = messageSpans.get(message)
      if (!span) return
